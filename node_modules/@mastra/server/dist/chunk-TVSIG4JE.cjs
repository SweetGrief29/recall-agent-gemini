...
var googleFailedResponseHandler = chunkSIGXR3JT_cjs.createJsonErrorResponseHandler({
...
var GoogleGenerativeAILanguageModel = class {
  ...
  async getArgs({
    ...
    const warnings = [];
    const googleOptions = chunkSIGXR3JT_cjs.parseProviderOptions({
      provider: "google",
  ...
  async doGenerate(options) {
    ...
    const mergedHeaders = chunkSIGXR3JT_cjs.combineHeaders(
    ...
    const {
      ...
      successfulResponseHandler: chunkSIGXR3JT_cjs.createJsonResponseHandler(responseSchema),
  ...
  async doStream(options) {
    const { args, warnings } = await this.getArgs(options);
    const body = JSON.stringify(args);
    const headers = chunkSIGXR3JT_cjs.combineHeaders(
      await chunkSIGXR3JT_cjs.resolve(this.config.headers),
      options.headers
    );
    const { responseHeaders, value: response } = await chunkSIGXR3JT_cjs.postJsonToApi({
      ...
      body: args,
      failedResponseHandler: googleFailedResponseHandler,
      successfulResponseHandler: chunkSIGXR3JT_cjs.createEventSourceResponseHandler(chunkSchema),
      abortSignal: options.abortSignal,
      fetch: this.config.fetch
    });
    const { contents: rawPrompt, ...rawSettings } = args;
...
function createGoogleGenerativeAI(options = {}) {
  var _a;
  const baseURL = (_a = chunkSIGXR3JT_cjs.withoutTrailingSlash(options.baseURL)) != null ? _a : "https://generativelanguage.googleapis.com/v1beta";
  const getHeaders = () => ({
    "x-goog-api-key": chunkSIGXR3JT_cjs.loadApiKey({
  ...
  const createChatModel = (modelId, settings = {}) => {
    ...
    return new GoogleGenerativeAILanguageModel(modelId, settings, {
      ...
      baseURL,
      headers: getHeaders,
      generateId: (_a2 = options.generateId) != null ? _a2 : chunkSIGXR3JT_cjs.generateId,
...
//# sourceMappingURL=chunk-TVSIG4JE.cjs.map
//# sourceMappingURL=chunk-TVSIG4JE.cjs.map